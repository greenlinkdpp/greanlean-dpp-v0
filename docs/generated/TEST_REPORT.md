...
## 2026-09-06 电池法规 v2.0 增量测试

| 门禁 | 结果 |
| --- | --- |
| 迁移/映射生成 | 71 个数据点、213 条分类适用性、12 个扩展存储字段生成成功 |
| TypeScript | `tsc --noEmit` 通过 |
| 单元与集成回归 | 126/126 通过 |
| 类别计数 | EV、LMT、工业的六类状态与来源 PDF 表格一致 |
| 公开权限 | 公开投影不读取单体运行遥测；授权投影保留动态数据 |

迁移 0026 尚未在目标 Supabase 执行；数据库 verify 和真实页面 smoke 必须在安装后再次完成。
