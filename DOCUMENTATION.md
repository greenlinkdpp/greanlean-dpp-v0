...
统一后台与发布收口阶段：规范发布读取已切换到 `CANONICAL`，真实文件证据、发布门槛、Registry TEST、动态电池数据和外部完整性边界已接入固定 Preview。

## 2026-07-26 进度更新

- 四个参考产品完成中性化数据整理，演示/测试措辞不再作为公开产品声明；
- 发布审核提交改为幂等，同一候选的重复请求不会再触发开放审核唯一键错误；
- 电池准备度统一计算静态字段与授权动态字段，不再在不同后台区域显示冲突数字；
- 新增统一证据文件中心：真实上传、25 MB 限制、私有存储、不可变版本、SHA-256、字段多选映射和授权下载；
- 电池证据和核验状态改为由文件关系及审核状态生成，删除手工选择“已上传/已核验”的入口；
- 文件存在 SHA-256 仅表示完整性，不再自动等同为合规证据已核验；
- 删除普通文档 URL 和手填证据引用 ID 的重复后台入口；
- 新增 Registry 与区块链集成中心，严格绑定当前不可变发布版本；
- 没有已验证真实区块链连接器时禁用锚定，不生成交易 Hash；
- 本轮 `84` 项业务/集成测试、TypeScript 和 Next.js production build 通过；
- 最新固定 Preview：`https://greanlean-dpp-v0-songweinie-1175-songweinie-1175s-projects.vercel.app`。

当前外部依赖：

- EU Registry 正式电池语义目录及 PRODUCTION 凭据尚未提供；
- 尚未配置真实区块链节点、签名服务和外部密钥管理器；
- 尚未接入生产 BMS/EMS/网关厂商。
